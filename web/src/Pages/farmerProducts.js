...
class farmerProducts extends Component {
  ...
  render() {
    ...
    const farmerProductsList = products ? (products.filter(product => product.farmerId == user.id)) : null//filtering the products that login user owns
    const farmerProductArr = farmerProductsList.map(product => {//passing product to FarmerProduct one by one
...
export default connect(mapStateToProps, { getProducts, addProduct, updateStateRed })(farmerProducts)