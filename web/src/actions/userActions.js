...
export const addUser = (user) => (dispatch) => {
  return new Promise((resolve, reject) => {
    ...
    axios
      .post(`${ip}/users`, user)
      .then((res) => {
        // calling the post server call of the set url while passing the user object param as data
        ...
        if (token) {
          ...
          dispatch({
            //setting the dispatch action type and payload which will be invoked as a callback
            type: "ADD_USER",
            payload: user,
...
export const login = (credentials) => (dispatch) => {
  return new Promise((resolve, reject) => {
    if (credentials.id) {
      ...
      axios
        .post(`${ip}/users/${credentials.email}`, credentials)
        .then((res) => {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (token) {
          ...
          const currentUser = jwt.decode(token);
          if (res.status === 200) {
            //checking if the status property of the response header is 200(success)
            dispatch({
              type: "LOGIN",
              payload: currentUser,
            });
